...
class KnightsBuildingsContent extends Component {
  state = {
    ...
    columnsStructure: {
      ...
      col_durability: { title: 'Wytrzymałość', dataIndex: 'durability', align: 'center' },
      col_description: { title: 'Działanie', dataIndex: 'description' },
      col_worker: { title: 'Pracownik', dataIndex: 'worker', align: 'center' },
  ...
  render() {
    ...
    const { col_name, col_cost, col_fields, col_durability, col_description, col_worker, col_image } = this.state.columnsStructure;
    ...
    tableColumns = [
      {
        ...
        render: (name) => (
          <>
            <h3><strong>{name[0]}</strong></h3>
            <Img className={componentClassnames.images.icon} src={require(`../${general}/${section.knights}/${dir.icons}/${name[1]}.${format.png}`)} />
          </>),
      },
      {
        title: col_cost.title, dataIndex: col_cost.dataIndex, align: col_cost.align,
        render: (cost) => {
          return (
            <p>{cost[0]} <Img className={componentClassnames.images.material} src={require(`../${general}/${section.knights}/${dir.materials}/deska.${format.png}`)} />
              <br />
              {cost[1]}<Img className={componentClassnames.images.material} src={require(`../${general}/${section.knights}/${dir.materials}/kamien.${format.png}`)} /></p>
          )
        }
      },
      { title: col_fields.title, dataIndex: col_fields.dataIndex, align: col_fields.align, },
      { title: col_durability.title, dataIndex: col_durability.dataIndex, align: col_durability.align, },
      {
        title: col_description.title, dataIndex: col_description.dataIndex,
        render: (description) => {
          ...
          return (
            <span>
              {description.map((fragment, index) => {
                if (fragment) {
                  let materialElements = fragment.match(textBetweenTagsRegEx);
                  return (
                    <p key={fragment}><strong>{descriptionSubHeaders[index]}</strong>
                      {materialElements ? materialElements.map((element, index) => <Img className={componentClassnames.images.material} src={require(`../${general}/${section.knights}/${dir.materials}/${element}.${format.png}`)} />) : fragment}
                    </p>
                  )
                } else return null
              })}
      ...
      {
        title: col_worker.title, dataIndex: col_worker.dataIndex, align: col_worker.align,
        render: (worker) => {
          let workersElements = worker.match(textBetweenTagsRegEx)
          console.log(workersElements);
          return (workersElements ? <Img className={componentClassnames.images.unit} src={require(`../${general}/${section.knights}/${dir.units}/${workersElements}.${format.PNG}`)} /> : null)
        }
      }, {
        title: col_image.title, dataIndex: col_image.dataIndex, align: col_image.align,
        render: (image) => <Img className={componentClassnames.images.building} width="120px" height="100px" src={require(`../${general}/${section.knights}/${dir.buildings}/${image}.${format.bmp}`)} />
      }
    ]
